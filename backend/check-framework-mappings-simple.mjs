...
async function checkFrameworkMappingsSimple() {
  console.log('🔍 Checking Framework Question Mappings (Simple)...\n');
  ...
  try {
    ...
    if (aError) {
      ...
      // Check each assessment's framework
      for (const assessment of assessments) {
        ...
        const { data: questionMappings, error: qmError } = await supabase
          .from('framework_question_map')
          .select('id')
          .eq('framework_version_id', assessment.framework_version_id);
        
        if (qmError) {
          console.log(`     ❌ Error checking questions: ${qmError.message}`);
        } else if (questionMappings && questionMappings.length === 0) {
